...
function luckyNumber(number) {
    ...
    let happy_arr = number.filter(function(num) {
        return (num == 4 || num == 7);
    });
    let happy_count = happy_arr.length.toString().split('');
    let is_happy = happy_count.filter(function(num) {
        return (num == 4 || num == 7);
    });
    if (happy_count.length == is_happy.length) {
        return true;
    } else {
        return false;
    }
